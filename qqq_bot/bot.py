        ...
        try:
            ...
            if price and price > 0:
                ...
                for br in app.brokers:
                    ...
                    if contracts >= 1:
                        # ВСЕ приказы — рыночные (и вход, и выход). Причина:
                        # лимитка зависает, когда цена уходит — на входе пропускаешь
                        # движение (сигнал был, а позиции нет), на выходе не закрываешься.
                        # Раз стратегия дала сигнал — исполняем гарантированно по рынку.
                        # Цена проскальзывания < цены пропущенной/зависшей сделки.
                        use_market = True
                        if is_auto:
...
async def _setup_command_menus(bot, app) -> None:
    ...
    full = _cmds([
        ...
        ("diag", "🔬 Диагностика (/diag или /diag2)"),
